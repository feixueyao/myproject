...
const page = {
	...
	tofit:function(){
		if(this.title.length>0){
			...
			$('.tea-content').css('height','4.68rem')
		}
...
// 答题同步
let answer = configData.source.words.split('#').join('')
let result = ''
let wordsClick = true
$('.words').on('click','li',function(e){
	if(wordsClick){
		wordsClick = false
		if(!isSync){
...
$('.sentence').on('click','span',function(e){
	if(sentenceClick){
		sentenceClick = false
		if(!isSync){
...
$('.container').on('syncSubmit','.submit.active',function(e,message){
	...
	isRight?$('.sentence span').css('color','#56a200'):$('.sentence span').css('color','#ff4848')
	...
	if(isSync){
		SDK.bindSyncResultEvt({
			...
			resultData:{result:result,isRight:isRight},
			syncName:'bShowResult'
		})
	}
	// submitClick = true	
	wordsClick = false
...
let statics = {}
$('#container').on('bShowResult',function(e,message){
	//旧版 let stuId = obj.user.id
	let sendUserInfo = message.data[0].value.sendUserInfo
    let resultData = message.data[0].value.syncAction.resultData
    let stuId = sendUserInfo.id
	statics[stuId] = {}

	if(sendUserInfo.type == 'stu'){
		statics[stuId].name = sendUserInfo.name
		statics[stuId].result = resultData.result
		statics[stuId].isRight = resultData.isRight
	}

	let eles = ''
	for(let key in statics){
		eles += `<li>
              <div class="name ${statics[key].isRight?'correct':'error'}">${statics[key].name}</div>
              <div class="result">${statics[key].result}</div>
            </li>`            
	}
	$('.tea-answers ul').empty().append(eles)
	SDK.setEventLock()
})
