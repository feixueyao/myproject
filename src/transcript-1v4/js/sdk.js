    ...
    if (h5SyncActions && h5SyncActions.isSync) {
        ...
        var sdk = {
            ...
            updateUser: function (message) {
                console.log('sdk-----updateUser------------>%s', JSON.stringify(message));
                //判断是否需要进行变更操作
                var bind_updateUser = $('.container').attr('data-bind-updateuser');
                console.log('sdk----------bind_updateUser---------->%s', bind_updateUser);
                if(bind_updateUser == '1') {
                    console.log('sdk-----需要触发用户变更效果');
                    $('.container').trigger('updateUser', message);
                    setTimeout(function () {
            ...
            //星星结果
            starEvent: function (message) {
                console.log('sdk-----starEvent------------>%s', JSON.stringify(message));
                var bind_star = $('.container').attr('data-bind-star');
                if(bind_star == '1') {
                    $('.container').trigger('syncStarsResult', message);
                }
            }
