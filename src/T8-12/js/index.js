...
$(() => {
  ...
  wrapper.on("syncItemClick", ".item", function(e,message) {
    console.log('========================同步点击啦========')
    var _this = $(this),
    ...
    if (firstCard) {
      let _firstCard = firstCard;
      firstCard = null;
      if (_this.data("id") == _firstCard.data("id")) {
        _this.addClass('active');
        $(this).queue(function() {
          _mask = mask.replace(/{{img}}/g, img == "" ? _firstCard.data("img") : img)
            .replace(/{{audio}}/g, audio)
            .replace(/{{text}}/g, text == "" ? _firstCard.data("text") : text);
          ...
          _firstCard.css("visibility", "hidden");
          _this.css("visibility", "hidden");

          //增加是否可提交标记
          _firstCard.attr('pass',true);
          _this.attr('pass',true);
          ...
          $(this).dequeue();
          //全部消除之后
          if( $('.item[pass=true]').length == source.items.length*2 ){
            console.log('全部消除啦！发送数据！！！');
            if (isSync) {
        ...
        _firstCard.removeClass("active");
        _this.addClass("error shake");
        _firstCard.addClass("error shake");
        $(this).delay(500).queue(function() {
          _firstCard.removeClass("shake");
          _this.removeClass("shake");
          $(this).dequeue();
        }).delay(0).queue(function() {
          _firstCard.removeClass("error");
