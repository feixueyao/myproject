var domain = 'http://172.16.0.107:9011/api/backend/';
//var domain = '';
var Data = {
    configData: {
        bg: "",
        // isAnswer:true,
        des: "description-When do we have breakfast?",
        tg: [{
            title: "when do we have breakfast",
            content: "when do we have breakfastwhen do we have breakfastwhen do we have breakfastwhen do we have breakfast",
        }, {
            title: "when do we have breakfast",
            content: "when do we have breakfastwhen do we have breakfastwhen do we have breakfastwhen do we have breakfast",
        }],
        source: {
            title: "scription-When do we h",
            text: "breakfast",
            image: "",
            audio: ""
        }
    ...
    methods: {
        ...
        imageUpload: function(e, item, attr) {
            var file = e.target.files[0],
                ...
                naturalHeight = -1,
                that = this;
            if ((size / 1024).toFixed(2) > 200) {
                console.error("您上传的图片大小为：" + (size / 1024).toFixed(2) + "KB");
                alert("您上传的图片大小为：" + (size / 1024).toFixed(2) + "KB, 超过200KB上限，请检查后上传！");
                return;
            }
            item[attr] = "./img/loading.jpg";
            var img = new Image();
            img.onload = function() {
